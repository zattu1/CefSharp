...
namespace CefSharp.fastBOT.Core
{
    ...
    public class BrowserTabManager : IDisposable
    {
        ...
        // タブ幅の設定
        private const double FIXED_TAB_WIDTH = 200.0; // 固定タブ幅
        private const double MIN_TAB_WIDTH = 120.0;   // 最小タブ幅
        private const double MAX_TAB_WIDTH = 250.0;   // 最大タブ幅
        ...
        public BrowserTab CreateTab(string title, string url, IRequestContext requestContext = null)
        {
            try
            {
                ...
                // タブヘッダー用のStackPanel作成（固定幅設定）
                var headerPanel = new StackPanel
                {
                    Orientation = Orientation.Horizontal,
                    VerticalAlignment = VerticalAlignment.Center,
                    Width = FIXED_TAB_WIDTH, // 固定幅を設定
                ...
                var titleTextBlock = new TextBlock
                {
                    ...
                    Width = FIXED_TAB_WIDTH - 30, // Faviconとマージンを考慮
                ...
                var tabItem = new TabItem
                {
                    Header = headerPanel,
                    Content = browser,
                    Width = FIXED_TAB_WIDTH, // TabItem自体にも固定幅を設定
                ...
                Console.WriteLine($"Tab created: {title}");
                return tab;
            }
            catch (Exception ex)
            {
        ...
        private void OnFrameLoadEnd(BrowserTab tab, IFrame frame)
        {
            try
            {
                if (frame.IsMain)
                {
                    ...
                    // 現在のアクティブタブの場合、MainWindowのURLを再同期
                    if (GetCurrentTab() == tab)
                    {
                        SyncUrlToMainWindow(tab);
                    }
                ...
                Application.Current.Dispatcher.Invoke(() =>
                {
                    try
                    {
                        if (args.IsLoading)
                        {
                            ...
                            {
                                var titleBlock = headerPanel.Children[1] as TextBlock;
